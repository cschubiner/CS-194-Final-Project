...
def delete_task(task_id, group_id):
	query_result = db_session.query(models.Task).filter(models.Task.id==task_id).first()
	if query_result:
		db_session.delete(query_result)
		db_session.commit()
		updated_query = db_session.query(models.Task).filter(models.Task.group_id==group_id).order_by(models.Task.date).all()
